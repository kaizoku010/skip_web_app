    ...
    try {
      ...
      const all_attendees = events.flatMap((event) => {
        return event.attendees.filter((attendee) => {
          // Ensure both attendeeId and userId are strings for comparison
          return attendee.attendeeId === String(user?.userId);
        });
      });
  
      // Update state or perform any operation with the filtered attendees
      setAllAttended(all_attendees);
  
      // Return all attendees for further use, if needed
