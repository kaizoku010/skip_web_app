...
const AuthProvider = ({ children }) => {
  ...
  const socket = io('https://skip-api-1gup.onrender.com'); 
  ...
  onst sendMessage = async () => {
    if (!newMessage || !selectedRoom) return;

    const message = {
      messageId: 'unique_message_id', // Generate a unique ID (could use UUID)
    ...
    try {
      await axios.post(`/chat_rooms/${selectedRoom}/messages`, {
        senderId: 'your_user_id', // Replace with actual user ID
        content: newMessage,
      });
