...
const AuthProvider = ({ children }) => {
  ...
  try {
    if (user && userEventID) {
      // Fetch posts for the current event and user
      const response = await axios.get("https://skip-api-1gup.onrender.com/get_all_posts", {
        ...
        data: {
          ...
          eventId: userEventID.eventId, // event id from context
        }
      });
...
useEffect(() => {
...
}, [user, userEventID, events]); // Re-fetch when user or event changes
  ...
  return (
    <AuthContext.Provider value={{checkout_status, checkoutJob, events, user, login, logout, loading, error }}>
      {children}
