...
// People who sent you requests
const getMatchingAttendees = (attendees, chatRequests) => {
  // Extract the receiverIds from chatRequests
  const receiverEmails = chatRequests?.map(request => request?.senderId);
  
  // Filter the attendees based on whether their email is in receiverEmails
  const matchingAttendees = attendees.map(attendee => {
    if (receiverEmails?.includes(attendee.userEmail)) {
      // Find the matching request for this attendee
      const matchingRequest = chatRequests.find(request => request.senderId === attendee.userEmail);
      
      // Return the attendee with requestID added
      return {
        ...attendee,
        requestID: matchingRequest?.requestID,  // Add requestID to attendee
      };
    }
    return attendee;  // Return attendee as is if no match
  }).filter(attendee => attendee.requestID); // Filter out attendees without requestID
  
  setRequstesFiltred(matchingAttendees);
  ...
  const acceptRequest = async (requestId) => {
    try {
      const response = await fetch(`https://skip-api-1gup.onrender.com/accept_chat_req/${requestId}`, {
        method: "PUT",
        headers: {
      ...
      //     prevUsers.filter((user) => user.id !== requestId) // Remove accepted request from the list
      //   );
      // }
    } catch (error) {
  ...
  const declineRequest = async (requestId) => {
    try {
      ...
      return await response.json();
      // if (result.message === "Chat request declined") {
      //   setFoundUsers((prevUsers) =>
      //     prevUsers.filter((user) => user.id !== requestId) // Remove declined request from the list
      //   );
      // }
    } catch (error) {
