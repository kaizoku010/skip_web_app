...
const AuthProvider = ({ children }) => {
  ...
  const handleToggleLike = async (postId) => {
    ...
    const acceptRequest = async (requestId) => {
      try {
        const response = await axios.put(`https://skip-api-1gup.onrender.com/accept_chat_req/${requestId}`, {
          action: 'accept',
        });
        ...
        if (chatRoom) {
          // Add new chat room to state
          setChatRooms((prevRooms) => [...prevRooms, chatRoom]);
  
          // Remove accepted request from chat requests
