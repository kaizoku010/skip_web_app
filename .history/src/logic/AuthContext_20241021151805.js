...
const AuthProvider = ({ children }) => {
  ...
  const fetchSentChatRequests = async () => {
    try {
      const response = await axios.get(`https://skip-api-1gup.onrender.com/get_sent_chat_reqs/${user.userId}`);
     
      setSentChatRequests(response.data);
    const testRequests = response.data
    console.log("sent requests: ", testRequests )
    } catch (error) {
      console.error("Error fetching sent chat requests:", error);
    }
  };

// Debugging logs to check the data before processing
console.log("Events: ", events);
console.log("User ID: ", user?.userId);

const all_attendees = events.flatMap((event) => {
  // console.log(`Checking event: ${event.eventName}`);
  // console.log("Attendees in event: ", event.attendees);

  // Make sure the attendeeId and userId are of the same type (e.g., both are strings)
  return event.attendees.filter((attendee) => {
    console.log(`Checking attendeeId: ${attendee.attendeeId} against userId: ${user?.userId}`);
    return attendee.attendeeId === user?.userId;
  });
});

// console.log("All attendees matching the user: ", all_attendees);
