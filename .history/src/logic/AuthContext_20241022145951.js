...
const AuthProvider = ({ children }) => {
  ...
  const fetchChatRooms = async () => {
    try {
      const response = await axios.get(`https://skip-api-1gup.onrender.com/chat_rooms/${user.userEmail}`);
  ...
  const fetchSingleRoom = async()=>{
    try {
      const response = await axios.get(`https://skip-api-1gup.onrender.com/chat_roo/${user.userEmail}`);
