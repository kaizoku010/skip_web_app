...
const AuthProvider = ({ children }) => {
  ...
  const createChatRoom = async (name, participants) => {
    try {
      const response = await fetch('https://skip-api-1gup.onrender.com/create_chat_room/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        notification.success({
          ...
          description: 'A new chat room has been created successfully.',
        });
  
        addChatRoomToCollapsible(result);
      } else {
        notification.error({
      ...
      notification.error({
        message: 'Error',
        description: 'There was an issue creating the chat room.',
      });
    }
  };
