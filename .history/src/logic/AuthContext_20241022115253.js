...
const getMatchingAttendees = (attendees, chatRequests) => {
  // Extract the senderId and requestID from chatRequests
  const matchingAttendees = attendees.map(attendee => {
    // Find the chat request where the senderId matches the attendee's userEmail
    const matchingRequest = chatRequests?.find(request => request.senderId === attendee.userEmail);
    
    // If a match is found, add the requestID to the attendee
    if (matchingRequest) {
      return {
        ...attendee,
        requestID: matchingRequest.requestID,  // Add the requestID
      };
    }
    
    // If no match, return the attendee as is
    return attendee;
  }).filter(attendee => attendee.requestID);  // Only include attendees with a matching request
