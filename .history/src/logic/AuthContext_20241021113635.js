...
const AuthProvider = ({ children }) => {
  ...
  const fetchSentChatRequests = async (userId) => {
    try {
  ...
  const sendChatRequest = async (recieverId, senderId) => {
    try {
      const response = await axios.post(`https://skip-api-1gup.onrender.com/chat_request`, {
        recieverId,
        senderId,
      });
      console.log('Chat request sent response:', response.data);
    } catch (error) {
