...
const AuthProvider = ({ children }) => {
 ...
 // Accept chat request and create a chat room
    ...
    try {
      ...
      if (result.chatRoom) {
        ...
        notification.success({
          ...
          description: 'You have successfully accepted the chat request and a chat room has been created.',
        });

        // Add the new chat room to the state
        setChatRooms((prevRooms) => [...prevRooms, result.chatRoom]);
      ...
      notification.error({
        message: 'Error',
        description: 'There was an issue accepting the chat request.',
      });
    }
  };
    ...
    try {
      ...
      if (result.message === "Chat request declined") {
        notification.success({
          ...
          description: 'You have successfully declined the chat request.',
        });

        // Optionally remove the declined request from the list
        setChatRequests((prevRequests) =>
          prevRequests.filter((request) => request.requestId !== requestId)
        );
      }
      ...
      notification.error({
        message: 'Error',
        description: 'There was an issue declining the chat request.',
      });
    }
  };
  ...
  return (
    <AuthContext.Provider 
      value={{
        ...
        chatRooms,
        chatRequests,
        setChatRequests, // To allow components to update chat requests
        acceptRequest,
        declineRequest,
        deletePost, 
