...
// Example function to calculate unread count
const calculateUnreadCount = () => {
  if (!messages || !lastReadMessageId[chatRoomId]) return 0;

  const lastReadIndex = messages.findIndex(msg => msg.messageId === lastReadMessageId[chatRoomId]);

  if (lastReadIndex === -1) return messages.length; // All messages are unread if lastReadMessageId is not found

  // Count unread messages after the last read message index
  return messages.slice(lastReadIndex + 1).length;
};

// Use the calculated unread count
const unreadCount = calculateUnreadCount();

useEffect(() => {
  console.log(`Total messages: ${messages.length}, Last read: ${lastReadMessageId[chatRoomId]}`);
  console.log(`Unread count for room ${chatRoomId}: ${unreadCount}`);
}, [messages, lastReadMessageId, chatRoomId]);
