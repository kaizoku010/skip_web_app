...
const sendMessage = async () => {
  ...
  // Get the current user's email (assuming user object has the email)
  const currentUserEmail = user.email; // Adjust this based on how you're storing the user's info

  // Find the receiver's email by filtering out the current user's email from participants
  const receiverEmail = selectedRoom.participants.find((email) => email !== currentUserEmail);
console.log("Reciever ID: ",receiverEmail );
  if (!receiverEmail) {
    console.error("Receiver email not found!");
    return; // Exit if receiver email is not found
  }
