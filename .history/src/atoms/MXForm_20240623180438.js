...
function MXForm({ id, onRegistrationSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const username = `user_${uuidv4().substring(0, 8)}`; // Unique non-email username
      ...
      const attributeList = [
        ...
        new CognitoUserAttribute({ Name: 'preferred_username', Value: username }) // Set a unique username
      ];
