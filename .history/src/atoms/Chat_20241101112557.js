import { Avatar, Collapse, Modal, Input, Button, Popover , notification, Divider, Badge  } from "antd";
...
const Chat = () => {
  ...
  const openChat = (room) => {
    ...
    setLastReadMessages((prev) => ({
      ...prev,
      [room.chatRoomId]: messages.length, // Save current message count
    }));
  };

  const getUnreadCount = (room) => {
    const lastReadCount = lastReadMessages[room.chatRoomId] || 0;
    const unreadCount = (room.messages?.length || 0) - lastReadCount;
    console.log(`Unread count for room ${room.chatRoomId}:`, unreadCount);
    return unreadCount;
  };
                ...
                {uniqueChatRooms?.length > 0 ? (
                  uniqueChatRooms.map((room) => {
                    ...
                    return otherParticipants.map((participantEmail) => {
                      ...
                      return (
                    <div className="chat-room-item" onClick={() => openChat(room)}>
  <Avatar className="sf-image" src={participantInfo?.userImage || TestImg} />
...
{getUnreadCount(room) > 0 && (
  <Badge count={getUnreadCount(room)} style={{ backgroundColor: "#52c41a", position: 'absolute', top: '10px', right: '10px' }} />
)}
