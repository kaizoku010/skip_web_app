...
function Checkout({eventPrice, eventId}) {
  ...
  const handleEventSignup = async (user_Id) => {
    if (!user) {
  ...
  return (
    <div className="checkout-container">
      ...
      <p style={{color:"blue", marginBottom:"0px"}}>{user?.userName || "Login"}</p>
      <p style={{color:"lightblue", marginBottom:"0px", fo}}>{user?.userEmail || "-/"}</p>
      <p style={{color:"lightblue", marginBottom:"0px"}}>{user?.phone || "-/"}</p>
