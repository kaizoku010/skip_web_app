...
function EventDetails() {
  ...
  const eventPrice = event => 2 ? eventPrice : "Free Entry"
  return (
...
<p className='ed-evnt-desc'><span className='eded-span'>Pricing: </span>{event?.eventPrice >= 2 ? eventPrice : "Free Entry"}</p>
...
<img className='ed-image' src={event?.eventImage}/>
      ...
      <div className="fixed-div">
       ...
       <Checkout eventPrice={event?.eventPrice} eventId={event?.eventId} eventName={event?.eventName}/>
       <p className='powered-payment'>Secure payments powerd by flutterwave.</p>
      </div>
    </div>
  );
}
