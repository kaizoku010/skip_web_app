...
function CurrentEvent({ user, events }) {
  ...
  const findAttendeeByEmail = (events, email) => {
    for (const event of events) {
      const attendee = event.attendees.find(attendee => attendee.userEmail === email);
      if (attendee) {
        return { eventName: event.eventName, attendee };
      }
    }
    return null; // Return null if no attendee found
  };

  const result = findAttendeeByEmail(userEvent, emailToFind);



  console.log("user event id:", user)
