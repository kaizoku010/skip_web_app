import { Avatar, Collapse, Modal, Input, Button, Popover , notification, Divider } from "antd";
...
const Chat = () => {
  ...
  const shareContact = async () => {
    ...
    const contactMessage = ` ${contactInfo.username} just shared their contact info with you, Email: ${contactInfo.userEmail}, Phone Number ${contactInfo.contact}`;
          ...
          {!messages.length ? (
           ...
           style={
             msg.messageContent.includes("shared their contact info")
               ? { color: "white",     background-color: rgb(18 141 37 / 61%);
 } // Adjust text color and background color as needed
               : {}
           }
         >
