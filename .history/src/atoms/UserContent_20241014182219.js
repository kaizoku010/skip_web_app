    ...
    {posts.length === 0 ? (
      ...
      posts.map((post) => (
        <div className="post" key={post.postId}>
          ...
          {post.mediaUrl && <Avatar className="post-image-2" src={post.mediaUrl} alt="Post" />}
          <div className="comment-section">
            <p className="cs-text like_action">Like</p>
            <p className="cs-text comment_action">Comment</p>
            <p className="cs-text share_action">Share</p>
          </div>
        </div>
      ))
    )}
