...
const Chat = () => {
  const { all_attended, chatRooms, currentUser } = useContext(AuthContext); // Assuming currentUser contains the current user's email
                ...
                {chatRooms.length > 0 ? (
                  chatRooms.map((room) => {
                    ...
                    const otherParticipants = room.participants.filter(
                      (email) => email !== currentUser.userEmail
                    );
                    ...
                    return otherParticipants.map((participantEmail) => {
                      ...
                      return (
                        <div key={room.chatRoomId} className="chat-room-item">
                          <Avatar
                            className="sf-image"
                            src={TestImg} // You can replace this with participantInfo's image if available
                          />
