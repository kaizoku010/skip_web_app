...
function MXForm({ id, onRegistrationSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const code = uuidv4().substring(0, 6); // Generate new code

      setNewCode(code); // Set newcode state
      ...
      const attributeList = [
        ...
        new CognitoUserAttribute({ Name: 'occupation', Value: job }),
        new CognitoUserAttribute({ Name: 'office', Value: imageUrl })
      ];

      userPool.signUp(email, code, attributeList, null, async (err, result) => {
        if (err) {
