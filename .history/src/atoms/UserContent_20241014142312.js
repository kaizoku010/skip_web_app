...
function UserContent({ user, events }) {
  ...
  const { posts, user, fetchPosts, loading, error } = useContext(AuthContext); // Use the context to access posts
  ...
  useEffect(() => {
    fetchPosts(); // Fetch posts whenever the component is mounted or context changes
  }, [fetchPosts]);
